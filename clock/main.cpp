...
void clock_face() {
    ...
    // day border
    tool.set_pen_width(1.0f);
    tool.set_pen_color(color.edge);
    tool.fill_rect(xc + clock_radius * 0.4, yc - 10, 40, 20, color.day);
}
...
void clock_hand(int h, int m, int s) {
    ...
    const unsigned char* c_str = (const unsigned char*)day_str.c_str();
    tool.RenderString(xc + clock_radius * 0.4 + 11, yc + 7, c_str, color.text);
